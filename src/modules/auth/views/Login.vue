      ...
      <v-flex xs12 sm6 md4 lg3>
        <v-card class="elevation-12">
          ...
          <v-card-text>
            ...
            <v-btn block depressed color="accent" @click="isLogin = !isLogin">
              {{ texts.button }}
            </v-btn>
          </v-card-text>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" :disabled="$v.$invalid" @click="submit" large>{{ texts.toolbar }}</v-btn>
          </v-card-actions>
...
export default {
  ...
  computed: {
    ...
    nameErrors () {
      ...
      !name.minLength && errors.push(`Insira pelo menos ${name.$params.minLength} caracteres!`)
      return errors
    },
    emailErrors () {
      const errors = []
    ...
    passwordErrors () {
      ...
      !password.minLength && errors.push(`Insira pelo menos ${password.minLength} caracteres!`)
      return errors
    }
  },
  methods: {
    async submit () {
      this.isLoading = true
      try {
        // teste loading, aguarda 3 segundos
